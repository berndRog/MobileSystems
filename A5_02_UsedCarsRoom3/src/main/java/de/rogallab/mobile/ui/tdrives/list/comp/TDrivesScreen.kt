...
private const val TAG = "<-TDrivesScreen"
   ...
   Box(
      ...
      Column(modifier = Modifier.fillMaxSize()) {
         TopAppBar(
            ...
            title = { Text(stringResource(R.string.test_drives_title)) },
         )
...
private fun TDrivesList(
   ...
   LazyColumn(
      ...
      itemsIndexed(
         items = tDrives,
         key = { _, tDrive -> tDrive.id },
      ) { originalIndex, tDrive ->
         SwipeEditDeleteItem(
            itemKey = tDrive.id,
            ...
            onEdit = { onIntent(TDrivesIntent.Open(tDrive.id)) },
            onRemove = { onIntent(TDrivesIntent.Remove(tDrive, originalIndex)) }
         ) {
            val personName = people.firstOrNull { person ->
               person.id == tDrive.personId
            }?.displayName ?: stringResource(R.string.value_not_available)
            val carName = cars.firstOrNull { car ->
               car.id == tDrive.carId
            }?.displayName ?: stringResource(R.string.value_not_available)
            ...
            TDriveCard(
               ...
               carName = carName,
               onClick = {
                  onIntent(TDrivesIntent.Open(tDrive.id))
               },
            )
         }
      }
   ...
   Card(
      ...
      Row(
         ...
         Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
            Text(
               text = stringResource(
                  R.string.test_drive_card_title,
                  personName,
                  carName,
               ),
            ...
            Text(
               text = DateTimeText.format(tDrive.start),
               style = MaterialTheme.typography.bodyMedium,
            )
            Text(
               text = stringResource(
                  if (tDrive.isCompleted) {
                     R.string.test_drive_status_completed
                  }
                  else {
                     R.string.test_drive_status_planned
                  }
               ),
